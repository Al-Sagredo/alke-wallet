# Alke Wallet 

**Alke Wallet** es una aplicación de billetera digital interactiva desarrollada como el proyecto final del Módulo 2 de la evaluación de Fundamentos del Desarrollo Front-end. El objetivo principal de este MVP (Producto Mínimo Viable) es proporcionar una interfaz segura, responsiva y fácil de usar para la simulación de transacciones financieras digitales básicas.

##  Demostración en Vivo
Puedes interactuar con el prototipo funcional directamente desde tu navegador en el siguiente enlace:
**[Ver Demostración en Vivo - Alke Wallet](https://al-sagredo.github.io/alke-wallet/login.html)**  
Email: `user@wallet.com`
Contraseña: 1234

---

## Características Principales (MVP)
La aplicación cumple de forma estricta con los requerimientos generales y técnicos del proyecto:

* **Registro e Inicio de Sesión (`login.html`):** Interfaz de acceso seguro al sistema con validaciones dinámicas de credenciales.
* **Menú Principal (`menu.html`):** Tarjetas de resumen financiero con visualización en tiempo real del saldo disponible.
* **Administración de Fondos (`deposit.html`):** Formulario interactivo para realizar simulación de depósitos y actualización inmediata de saldos mediante almacenamiento local.
* **Envío de Fondos (`sendmoney.html`):** Módulo dinámico para la gestión de la agenda de contactos con funciones de búsqueda por coincidencia, selección visual con resaltado y formulario de transferencia.
* **Historial de Transacciones (`transactions.html`):** Registro de los últimos movimientos con un sistema de filtrado dinámico según el tipo de transacción (depósitos, compras o transferencias).

---

## Tecnologías Utilizadas
Para el desarrollo de la interfaz responsiva y la optimización de la experiencia de usuario (UX) se utilizaron las siguientes especificaciones técnicas:

* **HTML5:** Estructura de las páginas esenciales implementando etiquetas semánticas y de accesibilidad.
* **Bootstrap 5:** Framework CSS utilizado para agilizar el diseño adaptable a dispositivos móviles y componentes de interfaz limpios.
* **JavaScript (ES6) & jQuery:** Motores lógicos implementados para la manipulación dinámica del DOM, validaciones de formularios y manejo asincrónico de eventos sin recargas de página.
* **Git & GitHub:** Control de versiones estructurado mediante el uso de ramas de características (`feature/login`, `feature/depositos`, `feature/transacciones`) para garantizar buenas prácticas de integración continua.
